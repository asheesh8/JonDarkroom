    ...
    <header className="sticky top-0 z-50 surface-wood-dark border-b border-brass/25 backdrop-blur-sm">
      <nav className="shell flex h-16 items-center justify-between gap-4">
        ...
        <a
          ...
          className="btn-reset hidden rounded-sm border border-brass/50 px-5 py-2 text-sm text-cream hover:bg-brass/10 lg:inline-flex"
        >
        ...
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="grid h-10 w-10 place-items-center rounded-sm border border-brass/40 text-cream lg:hidden"
        >
        ...
        <div className="surface-wood-dark border-t border-brass/20 lg:hidden">
          <ul className="shell flex flex-col py-3">
            ...
            <li className="mt-2 px-3">
              <a
                href={`tel:${businessInfo.phoneHref}`}
                className="btn-reset w-full rounded-sm bg-gradient-to-b from-brass-light to-brass-dark px-4 py-3 text-sm font-medium text-espresso"
              >
