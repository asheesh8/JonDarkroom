...
export function MockNotice() {
  return (
    <div className="mb-6 flex items-start gap-3 rounded-lg border border-brass/30 bg-brass/10 p-4 text-sm text-cream/80">
...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-brass/20 bg-[#241810]/70 p-5 sm:p-6",
        className,
      )}
    >
      {children}
...
const inputBase =
  "w-full rounded-md border border-brass/25 bg-espresso/60 px-3.5 py-2.5 text-sm text-cream placeholder:text-cream/30 focus:border-brass focus:outline-none focus:ring-1 focus:ring-brass";
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          type="submit"
          disabled={saving}
          className="btn-reset rounded-md bg-gradient-to-b from-brass-light to-brass-dark px-5 py-2.5 text-sm font-medium text-espresso shadow-brass transition hover:-translate-y-0.5 disabled:opacity-60"
        >
